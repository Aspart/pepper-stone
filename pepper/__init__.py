...
def workflow(input_filename, template_filename, output_filename, rename_items=False, rename_events=False):
    ...
    if not os.path.exists(template_filename):
        ...
        logger.error(error)
        raise IOError(error)
    form_template = template_from_file(template_filename)
    form_wrapper = wrapper_from_file(input_filename)
    form_wrapper.process_to_file(form_template, output_filename, rename_items, rename_events)
