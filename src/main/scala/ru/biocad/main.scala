...
object main {
  ...
  def process(config: Config) = {
    ...
    val datasetHeaderBuilder = new DatasetMetaBuilder
    datasetHeaderBuilder.parseHeader(meta)
    val header = datasetHeaderBuilder.getHeader
    val ftm = header.framesToMerge
    val str = header.toString
    val data = getData(table)
    val dataset = new Dataset(header, data)
    val merged = dataset.getMerged
    val tmp = new Mapper
    val mapper = tmp.createMapper("/Users/roman/Projects/Biocad/clinical-raw/mapping/Common_data.csv")
    val mappedFrames = header.events.map{ e =>
      e.getFrameKeysByDescr(mapper.keys.toArray)
    }
    if(!config.split)
