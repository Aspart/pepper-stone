...
object ExportServant {
  def exportValues(ds: Dataset, baseOutPath: String): Unit = {
    val exportMap = ExportServant.magicSortHelper("/Users/roman/Projects/Biocad/clinical-sort")
    ...
    val tmp = data.map(x => ColumnParser.parse(x(0).trim) -> x)
      .groupBy(_._1.frame)
      .map(x => x._1 -> x._2.groupBy(_._1.versionedName))

    val nameHelper = ds.meta.frames.map(f => f.key -> f.description).toMap
  ...
  def exportFrames(ds: Dataset, baseOutPath: String): Unit = {
    val exportMap = ExportServant.magicSortHelper("/Users/roman/Projects/Biocad/clinical-sort")
  ...
  def exportFramesOrdered(ds: Dataset, baseOutPath: String): Unit = {
    val exportMap = ExportServant.magicSortHelper("/Users/roman/Projects/Biocad/clinical-sort")
    ...
    tmp.foreach { case (f, dat) =>
      ...
      val currentFile = new File(baseOutPath, f + " - " + nameHelper.getOrElse(f, f) + ".txt")
      val writer = new PrintWriter(currentFile)
      val tmp = dat.map(_._2.toSeq.sortBy(_._1.event.stripPrefix("E").toInt)).map(x => x.head._1 -> x).toSeq.sortBy(_._1.versionedName).map(_._2).map(_.map(_._2).transpose.map(_.toArray).toArray)
      val tmp2 = dat.map(_._2.toSeq.sortBy(_._1.event.stripPrefix("E").toInt).map(_._2))

      val tmpDataRows = dat.map(_._2.toSeq.sortBy(_._1.event.stripPrefix("E").toInt)).map(x => x.head._1 -> x).toSeq.sortBy(_._1.versionedName).map(_._2).map(_.map(_._2).transpose.map(_.toArray).toArray).toArray.transpose.map(_.reduce(_ ++ _))
      val dataRows = dat.map(_._2.toSeq.sortBy(_._1.event.stripPrefix("E").toInt).map(_._2).transpose.map(_.toArray).toArray).toArray.transpose.map(_.reduce(_ ++ _))
